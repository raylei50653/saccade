...
**感知快路徑 (Fast track)** 持續以 140+ FPS 運行，使用 YOLO11 與 TensorRT 引擎評估每一影格。透過 GStreamer (NVDEC) 實現零拷貝硬體解碼，數據全程保留在 GPU VRAM 中。

**語義提取慢路徑 (Slow track)** 運作於獨立的 CUDA Stream。當偵測到新物件或物件行為發生重大改變（語義漂移）時，使用 `roi_align` 在顯存中直接裁切，並透過 **Jina-CLIP-v2** (TensorRT) 提取 1024 維特徵向量存入 ChromaDB。

## 核心設計決策

**純視覺向量管線 (Pure Vision-Vector)** — 為了極大化吞吐量，我們從沉重的 VLM 轉向「YOLO + Jina-CLIP」架構。VRAM 佔用從 ~8GB 降至 **1.5GB**，效能提升數倍。
...
| 層級 | 技術 |
| :--- | :--- |
| 偵測 (Detection) | YOLO11 (TensorRT Engine), 實時物件追蹤 |
| 提取 (Extraction) | Jina-CLIP-v2 (TensorRT FP16, 512x512) |
