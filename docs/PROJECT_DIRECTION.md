...
> **這份文件是什麼**：專案級方向語義的唯一敘事面。它定義已確立的範式邊界、execution-chartered 的 project-level 目標、以及長期戰略路徑的進入/退出語義。
...
**時間：** 本檔只允許絕對歷史時間（寫在被引用 ADR/decision 上的 `YYYY-MM-DD`），不允許相對現在時間（目前 / 現在 / 近期 / 尚未 / 正在 / 接下來 / 已完成 / 截至 …）。live temporality 全部由 owner pointer 表達。不寫 `last updated`、`current as of`、milestone completion date。

**更新觸發**（其餘一律不更新；ordinary implementation PR 不得同步本檔）：

1. 新 ADR Accepted，改變範式或能力邊界（同 PR 附 pointer）
2. project-level objective 的 **execution charter 被掛上 / 卸下 / 替換 / 語義重定義**（例如正式轉向下一條全域 workstream）。這不是 ADR Accepted。**milestone 完成本身不觸發**——完成狀態由 `docs/TODO.md` 的 sole active 表示；generated status 若存在則見其 owner，不在本檔
...
## Project-level Strategic Objectives

此處列出 **execution-chartered** 或僅定義的 project-level 里程碑。本檔不複製 live execution status。execution-chartered 只表示 `docs/TODO.md` 掛了執行授權，**不是**對應 ADR 已 Accepted。
...
- **Execution-status pointer**：execution charter 見 [docs/TODO.md § Sole active](TODO.md)；ADR Status 見 [ADR 021](decisions/021-asset-provenance-and-progress-reporting.md)。本檔不複製這兩處，也不因 charter 完成而更新。
...
- **Execution-status pointer**：是否掛上全域 execution charter，見 [docs/TODO.md § Sole active](TODO.md)。**W-A 的 execution charter 被替換或卸下才更新本檔。**

不列為本節目標的例子：單模組 bugfix、perf 修復、未列入本節的 owner 決策候選（見 Runtime-grounded 路徑）。
...
- **Rationale**：geometry-first 是 ADR 019 所定義的 production 範式；ReID 在特定 regime（長遮擋、低幀率、跨鏡、鏡頭切斷、弱幾何）仍可能有研究價值。
- **Semantic entry**：出現**新的** appearance 模態或訊號源，且通過既有 revival / unlock 規則——不是重放 no_go 所記錄的 MOT17 appearance 路線。
...
- **Rationale**：streaming / storage / cognition / resource 有獨立模組家，但 accepted owner 所定義的系統形狀不是完整產品拓樸。
...
- **Semantic entry**：owner 將「H0 re-entry + actual baseline capture」或等價的 substrate 設計掛上 project-level execution charter。不列入本檔的 project-level 目標；是否 execution-chartered 由 execution-status owner 決定，見 [docs/TODO.md](TODO.md)。
