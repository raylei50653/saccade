...
- **特徵提取層 (Extraction):** 透過 `torchvision.ops.roi_align` 在 VRAM 內直接裁切，並餵入 SigLIP SO400M (TensorRT) 提取高維語義特徵。
...
| 資料夾 | 功能說明 |
| :--- | :--- |
| **`perception/`** | **視覺核心**。包含 YOLO 偵測器 (`detector.py`)、零拷貝裁切 (`cropper.py`)、SigLIP 特徵提取 (`feature_extractor.py`)、追蹤過濾 (`tracker.py`) 與語義去重 (`drift_handler.py`)。 |
...
| 資料夾 | 功能說明 |
| :--- | :--- |
| **`models/`** | **模型權重**。包含 YOLO (.pt, .engine) 與 SigLIP 的 TRT Engine (放置於 `embedding/`)。注意：二進制檔受 `.gitignore` 保護。 |

### 🧪 開發輔助 (Dev Tools)

| 資料夾 | 功能說明 |
| :--- | :--- |
| **`scripts/`** | **維運腳本**。包含一鍵啟動 CLI (`saccade`)、ONNX 導出 (`export_siglip.py`) 與 TRT 編譯 (`build_engine.py`) 腳本。 |
...
4.  **裁切與提取**: `perception/` 在背景 CUDA Stream 裁切新物件並提取 SigLIP 特徵。
