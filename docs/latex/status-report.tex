...
\newcommand{\sourcepath}[1]{%
  \href{https://github.com/raylei50653/saccade/blob/0fe937b4e7a4cfc8e142d4fdcca5cabf93b3e982/#1}{\path{#1}}}
...
\begin{titlepage}
  ...
  \begin{tabularx}{\linewidth}{@{}B{31mm}Y@{}}
    報告快照 & 2026-09-07\\
    對照版本 & \texttt{main 0fe937b4e7a4}（\pr{358} merge head）\\
...
MOTA & 偵測面總量 & 主要反映漏檢、誤檢與身份切換的總量，0--100 分，越高越好\\
...
品質輸出 bit-exact，不能計入 accuracy 增益。
...
handover 與 merge 的輸入只有已輸出的軌跡行，
因此\textbf{可以獨立解耦}；目前仍在同一流程的序列末呼叫，屬架構遺留而非演算法要求。
...
# Deterministic ReID / offline-handover A/B base
uv run python scripts/eval/mot17.py \
...
品質 bit-exact 不代表 timing bit-exact；FPS 必須以 paired block 或明確 protocol 處理系統漂移。
