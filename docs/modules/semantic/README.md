...
| 文件 | 內容 |
|------|------|
| [research/door0_ranking_probe_results_20260712.md](research/door0_ranking_probe_results_20260712.md) | **Door 0 probe results — terminal `T2 NO_USABLE_RANKING_POWER_IN_CLASS`（12-member tested class）** · V1–V5 PASS · H 未觸發（baseline PWA 0.878／top-1 0.590；P3=84/205 top-1 miss＝headroom 真實存在）· 12/12 candidates fail boxes（best ΔPWA=+0.0011，距 B1 差 20×）· 機制：class 可見 tail 已被 `s0` 排序（demote 無效），殘餘混淆在 in-band 聯合分佈內；motion conditions 淨有害（escape-tail 一致）· 無條件 reachable-set caveat · step ⑤ class-scoped closure；step ④ 未開 · packet [evidence/](research/evidence/door0_ranking_probe_20260712/manifest.json) · [thread](../../research/threads/ambiguous_band_ranking_power_probe_20260712.md) |
| [research/ambiguous_band_ranking_power_probe_declaration_20260712.md](research/ambiguous_band_ranking_power_probe_declaration_20260712.md) | **Door 0 — ambiguous-band ranking-power probe 預宣告（realignment step ③；seal = PR merge）** · §20.2 block（layer=score-ranking · intent=capability map · output=diagnostic only）· substrate=frozen pairs `0ae38967…`（205 rankable events）· coarse gate=production h-window proxy · baseline `s0=score_m_bridge` · 12 frozen candidates（6 singles+6 ANDs；q85/q15；lexicographic demotion，無 λ）· headroom H + boxes B1–B6 · validity V1–V5 · terminals T0–T3 全部映射主線 transition（§20.7）· [thread](../../research/threads/ambiguous_band_ranking_power_probe_20260712.md) |
