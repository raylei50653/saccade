...
| [research/discrete_m_capability_declaration_20260712.md](research/discrete_m_capability_declaration_20260712.md) | **discrete-\(M\) representation-capability 預宣告（sealed pre-outcome;owner seal pending）** · 問題＝「\(z_t\) 是否足以預測自己的短期未來」,**非** score fit · 新 capture contract `m0_state_capture_v1`（逐幀 per-track,read-only,default-off;R1 packet 只有四樣本窗,horizon 4/8 與 \(M^k\) 不可測,故不得沿用）· terminal 只認 \(z^{R}\)（R1 reduction）,\(z^{K}\) 僅 diagnostic · 五成員 affine family（identity／CV／diag／full／regime）由 one-step OLS 冪次外推 · metric＝\(h\)-normalized anchor error,LOO 七折 · 決策鏈 G0 validity → G1 ceiling 0.40（＝production threshold）→ G2 \(\rho(M)\le1.001\) → G3 ≥10 % over CV／≥6-of-7 folds／de-concentration ≥50 % · terminals `M0_INVALID`／`M_STATE_SUFFICIENT`／`CV_DOMINANT`／`STATE_INSUFFICIENT` · **不**授權 score／gate／preset／\(e^{A\Delta t}\) |
