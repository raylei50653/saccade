...
| 文件 | 內容 |
|------|------|
| [research/production_substrate_mapping_20260711.md](research/production_substrate_mapping_20260711.md) | **Production substrate mapping（Step-0 abstraction-chain audit · binding precondition for E3/A1–A8）** · `BOOLEAN_NATIVE_ALIGNED_WITH_GAPS`：relink 決策＝Boolean gate chain + appearance-only 排序（無 motion energy 聚合） · atom 四分類（exact / reconstructable / proxy-aligned / research-only） · consumer-specific support `S_relink=[2,45]`／`S_handover≤60`（row-level 切面；bin 31–60 橫跨 45） · deployment claim 須在 reachable support 單獨成立 · claim ladder 四級 · D0 `bridge_dist` fidelity gate（三值 verdict） · [thread](../../research/threads/gap_conditioned_probabilistic_motion_probe_20260711.md) |
