...
| 2026-09-13 | Monocular depth（YOLO26-depth）加入 relink cost gate / ReID 向量 / 靜態 camera calibration | ❌ NO-GO（naive fusion 兩次拖累既有 baseline：dist_h 39.8%→28.4%、bridge_dist 54.0%→33.3% rank-1）；ReID 向量架構上直接拒絕。後續 foreground-masked camera surface 雖穩定（temporal R² 0.876–0.971），但 neural tie-break 的所有 runtime 決策都被 cheap position→scale surface 完整複製，無 neural 增量。見 [monocular_depth_signal_survey_20260913](research/monocular_depth_signal_survey_20260913.md) |
