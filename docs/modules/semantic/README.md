...
| 2026-09-13 | Monocular depth（YOLO26-depth）加入 relink cost gate / ReID 向量 / 靜態 camera calibration | ❌ NO-GO（naive fusion 兩次拖累既有 baseline：dist_h 39.8%→28.4%、bridge_dist 54.0%→33.3% rank-1）；僅 direct depth concatenation into normalized ReID vector 在架構上拒絕，其他 multimodal interaction 未測。Foreground-masked camera surface 雖穩定（temporal R² 0.876–0.971），但在 tested top-two rule、margin 0.10/0.20/0.30 的 activated groups 中，neural candidate decisions 全被使用相同 GT-assisted calibration data 的 cheap position→scale control 複製。見 [monocular_depth_signal_survey_20260913](research/monocular_depth_signal_survey_20260913.md) |
...
| [research/headline_bridge_behavioral_identity_capture_declaration_20260725.md](research/headline_bridge_behavioral_identity_capture_declaration_20260725.md) | H2（proposed / draft-unsealed）：以 derivational + behavioral runtime identity 取代 H0 的枚舉式 physical provenance；Layer-P（pre-seal, retryable）／Layer-M（sealed, exactly-once）兩層預算；只 supersede H0 的 identity 層，H0 sealed history 全 byte-frozen |
...
| [research/evidence/h0_gctm_interface_static_feasibility_20260723/](research/evidence/h0_gctm_interface_static_feasibility_20260723/) | H0→GCTM bounded static feasibility packet：責任/coverage matrix、immutable derivations、schema/validator、conservation 與 mechanical `H0_GCTM_INTERFACE_STRUCTURALLY_INSUFFICIENT` terminal；非 runtime verdict |
| [research/evidence/gctm_runtime_native_candidate_universe_20260724/](research/evidence/gctm_runtime_native_candidate_universe_20260724/) | GCTM runtime-native candidate-universe consumer contract：event/candidate keys、pre-score inclusion、composition/completeness、registration-v3 requirements-only、mechanical `GCTM_RUNTIME_UNIVERSE_CONTRACT_SEALABLE`；非 H0 guarantee / 非 B1 activation |
| [research/evidence/h0_gctm_guarantee_registration_v3_20260724/](research/evidence/h0_gctm_guarantee_registration_v3_20260724/) | H0 registration-v3 universe-completeness contract：schema/validator、`event_universe_sidecar`、completeness predicate、candidate-source fixture、mechanical `H0_REGISTRATION_V3_CONTRACT_SEALABLE`；非 actual guarantee / 非 capture / 非 re-entry |
...
| 文件 | 這是什麼 |
|------|------|
| [research/observability_weighted_directional_likelihood_declaration_20260827.md](research/observability_weighted_directional_likelihood_declaration_20260827.md) | OWDL pre-outcome declaration：分開檢驗方向可觀測性現象與 held-out direction-channel ranking 缺口；**sealed 2026-09-20 at `311c2225`**（[seal receipt](research/observability_weighted_directional_likelihood_seal_receipt_20260920.json)）、thread parked、runner 未排程；非 runtime／MOT efficacy |
