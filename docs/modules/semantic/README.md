...
| 文件 | 內容 |
|------|------|
| [research/score_temporal_to_stable_domain_20260712.md](research/score_temporal_to_stable_domain_20260712.md) | **score 時域→穩定域建模（sole active）** · **R1 closed @ `R1_FAITHFUL`**；下一步＝**separately declared discrete-\(M\)** representation-capability study（**已草擬,待 owner seal**）。不重開 `s0`、不調 gate／權重／preset。對應 [active thread](../../research/threads/score_temporal_to_stable_domain_20260712.md)。 |
| [research/discrete_m_capability_declaration_20260712.md](research/discrete_m_capability_declaration_20260712.md) | **discrete-\(M\) anchor-propagation 預宣告（`draft`;owner seal pending,見 §12 seal event）** · 問題＝「lost-side \(z^{R}\) 的 **anchor** 能否比 production 已在用的 CV 規則傳播得更好」,**非** score fit、**不算** `bdist` · 新 capture contract `m0_state_capture_v1`（逐幀 per-track,read-only,default-off;R1 packet 只有四樣本窗,horizon 4/8 與 \(M^k\) 不可測,故不得沿用）· terminal 只認 \(z^{R}\),\(z^{K}\) 僅 diagnostic · pair universe 有凍結的 exclusion **precedence ladder** · 五成員 affine family + **pinned float64 SVD solver contract**（explicit `rcond`／\(\kappa_2\)／min-sample fail rules）· metric＝\(h\)-normalized **anchor-position** error,LOO 七折 · 決策鏈 G0 validity → **G1 stability eligibility 先行**（\(\rho\le1.001\) ∧ finite-horizon \(\|\tilde M^k\|_2\le2.0\) ∧ affine drift）→ G2 ceiling **四個 horizon 全過**（0.40 ＝ **production-inspired heuristic**,**非** accept-margin 等價）→ G3 ≥10 % over CV／≥6-of-7 folds／de-concentration ≥50 % · terminals `M0_INVALID`／`ANCHOR_STATE_INSUFFICIENT`／`ANCHOR_SHORT_HORIZON_ONLY`／`ANCHOR_M_SUFFICIENT`／`ANCHOR_BASELINE_DOMINANT`／`ANCHOR_M_REQUIRED_MARGIN_INSUFFICIENT` · **不**授權 score／gate／preset／\(e^{A\Delta t}\) |
