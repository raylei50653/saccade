...
🔄 **GT-Support Morphology — PR-D gate `GLOBAL_PARTIAL_ORDER_READY`; next = restricted-closure prototype**
...
- PR-D gate / [#106](https://github.com/raylei50653/saccade/issues/106) · [PR #107](https://github.com/raylei50653/saccade/pull/107): **`GLOBAL_PARTIAL_ORDER_READY`** · global=`{dist_h, log_h_ratio}` · `bridge_dist` conditional (motion-extrapolation) · motion conditional · `score_m_bridge`/`gap` context_only → [note](research/boolean_atom_partial_order_20260711.md) · [packet](research/evidence/boolean_atom_partial_order_20260711/manifest.json)
- Next: **separate restricted-closure prototype**（global pair only；vs frozen OR-tail；candidate-only；**after** #107 research acceptance）→ **PR-E** nested validation
...
- [gt-support morphology](../../research/threads/gt_support_morphology_20260711.md) — **ACTIVE**（sole active 本體）· PR-A/B/C sealed · PR-D gate `GLOBAL_PARTIAL_ORDER_READY` · next restricted-closure prototype
