...
- PR-D gate / [#106](https://github.com/raylei50653/saccade/issues/106): **`GLOBAL_PARTIAL_ORDER_READY`** · global=`{bridge_dist, dist_h, log_h_ratio}` · motion conditional · `score_m_bridge`/`gap` context_only → [note](research/boolean_atom_partial_order_20260711.md) · [packet](research/evidence/boolean_atom_partial_order_20260711/manifest.json)
- Next: **separate restricted-closure prototype**（global triple only；vs frozen OR-tail；candidate-only）→ **PR-E** nested validation
