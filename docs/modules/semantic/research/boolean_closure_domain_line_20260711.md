...
| Task 2 — Four-track forensic | Step-0 四條 $d\ge3$ GT tracks（**4/4 在 MOT17-10-SDP**，見 packet `tail_tracks.json`）逐件落入五個預宣告類別；判定 motion atom role reversal 是否具真實機制來源，並分辨單場景 vs 通用機制 | **PR-C / #102 recorded** · aggregate `ROLE_REVERSAL_SUPPORTED` · [note](escape_tail_forensic_20260711.md) · [packet](evidence/escape_tail_forensic_20260711/manifest.json) |
