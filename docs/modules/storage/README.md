...
| **輸出** | Redis Stream `saccade:stream` + List `saccade:events`（micro-batch 100ms / 50 條）、ChromaDB 向量持久化 |
| **上游 → 下游** | `perception events → MicroBatcher → Redis (rpush/xadd) → ChromaStore.hybrid_query / backup`；下游由 [cognition](../cognition/README.md) 讀取 |
