...
Total tracked scripts: **412**. Source of truth = each script's own docstring + `# status:` header; regenerate with `scripts/tools/build_scripts_index.py`.
...
| Label | Meaning | Count |
|-------|---------|------:|
| `stable` | supported workflow entrypoint; keep path stable | 152 |
| `diagnostic` | reusable analysis/debug tool, not a main workflow | 155 |
| `experiment` | tied to a named experiment or historical sweep | 99 |
