...
Total tracked scripts: **450**. Source of truth = each script's own docstring + `# status:` header; regenerate with `scripts/tools/build_scripts_index.py`.
...
| Label | Meaning | Count |
|-------|---------|------:|
| `stable` | supported workflow entrypoint; keep path stable | 178 |
| `diagnostic` | reusable analysis/debug tool, not a main workflow | 165 |
| `experiment` | tied to a named experiment or historical sweep | 101 |
