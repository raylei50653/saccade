...
Total tracked test files: **178**. Source of truth = each `test_*.py`'s own docstring + `# scope:` / `# function:` / `# lifecycle:` header; regenerate with `scripts/tools/build_tests_index.py`.
...
| `tests/research/misc/test_r1_temporal_reduction_capture.py` | eval | quarantined | R1 capture-contract tests: nested windows, fail-closed export, label-free replay. |
