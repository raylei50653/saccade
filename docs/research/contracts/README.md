...
| **[frozen_source_supersession_ledger_v1.json](frozen_source_supersession_ledger_v1.json)** | **CLOSED packet frozen input 的 historical / current 狀態（machine-readable）。** 規則在 [ADR 026](../../decisions/026-frozen-input-source-evolution.md)、執行在 `scripts/tools/frozen_source_status.py` 與 `tests/contract/test_frozen_source_evolution_policy.py`，本檔只承載 append-only 的轉移紀錄：哪個 `(path, sha256)` 自哪個 commit 起不再是 HEAD、被哪些 packet 綁、是 historicization 還是 owner 授權的 supersession。packet artifacts 與 H0 declaration 不可入帳；**缺檔 = 被刪除的 guard**。 |
