...
| [nogpudecode_reproducibility_20260907.md](nogpudecode_reproducibility_20260907.md) | **`--no-gpu-decode` 的重複執行變異**:重現性是 per-configuration 的,不是旗標給的(`baseline` 7-seq 14/20 分歧、`mamba_whole_graph_m --double-buffer` 0/20)＋取代「bit-exact ⇒ N=1」的判讀規則與受影響結論分級 |
