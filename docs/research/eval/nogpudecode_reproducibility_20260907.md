# `--no-gpu-decode` 的重複執行變異 — 重現性是 per-configuration 的 (2026-09-07)
...
3. **同旗標、同 host、同 session 下,組態之間的差異很大**:D 0/20 對 H 14/20。
...
在本文 block S／H 觀察到的分歧比例下,三跑全同是常見結果,**那份證據從一開始就不足以支撐該規則
的普遍形式** —— 即使該規則在 config D 上恰好沒有被本文推翻。
...
- n 是為了量指標離散度而定,**不估分歧機率**,本文不給比率。
- 未做任何機制量測。本文不說明哪個 buffer 被 race,也不說明 block D 為何沒出現。
- block 順序(S → H → D)未隨機化;session 內的漂移會表現成 block 差異。block D 排在最後,
  對「D 乾淨」這個結論是較不利的位置。
