...
| 規則 | 說明 |
|:--|:--|
| **一宇宙一表** | `U_det` / `U_gt` / `U_cand` / `U_err` 不混在同一 parquet |
...
- **維度 D** → 分層子宇宙 / 條件表  
- **AUC** → 固定 `(universe, y, score_field)`；必報 n%
...
| `CutDesign` | 多 method 實驗設計 |
| `DataSourceId` | 原始資料來源 |
| `StudyMeta` | study 契約（含 pipeline 欄位） |
| `GtRow` / `DetRow` / `CandRow` / `ErrRow` | 列型別 |
| `MethodMetricsRow` | 端到端 count 摘要 |
...
| `cut_design` | str | | `single` / `cumulative` / `single_on_base` / `orthogonal` / `custom` |
...
### 4.3 `U_cand` — `CandRow`（一列一個 (track, det) 候選）
