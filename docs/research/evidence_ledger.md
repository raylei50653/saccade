...
| 2026-09-24 | Merge-only identity repair, cross-dataset validation (#459) | **Gated conditionally, not global, not promoted** (`b3ac5725` substrates, `mamba_whole_graph_m`, `--double-buffer --no-gpu-decode`, fixed operating point): IDF1/HOTA/AssA up on MOT17 (+0.83), MOT20-01/02 (+1.04), DanceTrack (+3.48); DanceTrack MOTA −3.6 / FP +16.7k arises only with shipping interpolation after merge (interp-off MOTA +0.16); MOT20-03/05 merge OOM (dense S×S). MOT20 covers 2/4 sequences. No retune. | [merge_only_cross_dataset_20260924.md](../modules/semantic/research/merge_only_cross_dataset_20260924.md) · [#459](https://github.com/raylei50653/saccade/issues/459) |
| 2026-09-21 | #418 full-stack leave-one-out subtraction (`mamba_whole_graph` serial, `HEAD` = #421 production identity md5-for-md5) | **Interaction classification, no preset change**: 11/14 single removals material losses; occ_state×OAO, StabCost×StabBid, GMC×KalmanR, Bridge×Interp read as complement units (`I > 0`), Bridge×PrivCont as backup (`I < 0`); **DDA S0 stage and bridge reciprocal margin have no material role → simplification candidates**; `use_whole_graph` doubles as the T=1 selector on the temporal checkpoint (coupling to fix). Observed differences only; not additive. | [module_interactions_20260921.md](../reference/benchmarks/module_interactions_20260921.md) · [architecture/module_interactions.md](../architecture/module_interactions.md) |
