...
H2 successor identity line (proposed; authorizes nothing)
  ...
  seal additionally requires
    ...
    ∧ the published coordinate/probe current + controlled host green there
...
| [h2_behavioral_identity_capture_task.md](h2_behavioral_identity_capture_task.md) | mainline-study · **non-WIP** | H2 bridge-decision capture under behavioral runtime identity (successor to H0's identity layer; five spent `S` unchanged). Four owner decisions **accepted 2026-07-25** ([#286](https://github.com/raylei50653/saccade/issues/286)); S1–S3 landed, S4 Phase-A controller implemented and contract-tested. Current work = **review** the controller; do not seal or execute. Exact-head Layer-P certification, current coordinate/workflow green, and separate owner authorization remain gates | semantic |
