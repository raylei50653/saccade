...
| **NEXT** | **none automatically.** A sealable A-layer model spec is not an entry gate: B1 and O1 remain **proposed / non-WIP** behind their own gates below (accepted runtime substrate or fidelity edge, owner acceptance of the now-started but still-proposed cross-study [L2 score-layer contract draft](../contracts/score_ranking_evidence_contract.md) and its registry binding, the B1-slot identity, a sealed B1 declaration, and separate owner scheduling). | [GCTM charter (closed)](closed/gap_conditioned_stochastic_transition_model_task.md) · [B1 charter](gctm_b1_runtime_grounded_offline_attribution_task.md) · [O1 charter](gctm_o1_online_intervention_efficacy_task.md) |
...
GCTM → bridge-runtime B1
  ...
  ∧ owner-accepted score-layer L2 contract             ← draft started; not accepted
