...
| [h2_behavioral_identity_capture_task.md](h2_behavioral_identity_capture_task.md) | mainline-study · **non-WIP** | H2 bridge-decision capture under behavioral runtime identity (successor to H0's identity layer; five spent `S` unchanged). Four owner decisions **accepted 2026-07-25** ([#286](https://github.com/raylei50653/saccade/issues/286)); S1–S3 landed, S4 Phase-A controller implemented, contract-tested, reviewed and merged. Two authorized Phase-A invocations are **spent with zero capture**: `0a5dffe9…` (2026-07-27) → terminal 1, 0/4 runs, archive refused; `7646f421…` (2026-07-28, after the controller repair) → terminal 4, 1/4 runs, archive complete and verifier-accepted. Cause is now a child environment-validation ordering defect. Current work = **repair the child on a further successor head**, gated behind a non-evidence full run; acceptance items 4–5 must be rebuilt there, and both authorizations remain permanently spent — the owner must separately issue a third | semantic |
