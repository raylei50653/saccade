...
| [h2_behavioral_identity_capture_task.md](h2_behavioral_identity_capture_task.md) | mainline-study · **non-WIP** | H2 bridge-decision capture under behavioral runtime identity (successor to H0's identity layer; five spent `S` unchanged). Four owner decisions **accepted 2026-07-25** ([#286](https://github.com/raylei50653/saccade/issues/286)); S1–S3 landed, S4 Phase-A controller implemented, contract-tested, reviewed and merged. The one authorized Phase-A invocation was **spent at `0a5dffe9…` (2026-07-27) → terminal 1, 0/4 runs, no capture, no seal**; cause is a controller self-mutation defect. Current work = **repair the controller on a successor head**; acceptance items 4–5 must be rebuilt there, and item 6 remains permanently spent — the owner must separately issue a new authorization | semantic |
