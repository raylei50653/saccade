...
| **W4b — open** | **the child's callback-order invariant is the owner's to adjudicate.** `run_h2_measurement_child.py` is an execution-semantics member, so any change moves the RunSpec's declared content projection (it is not an `identity_semantics` member, so no re-pin or republish follows). Until it is settled no measurement can complete: every execution stops at the same point in run 00 | yes |
