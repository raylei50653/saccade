...
| **W6b** | 🔁 **two rehearsal reviews found two self-consistent verifier gaps; final rerun required.** The first disposable run at `84583eed…` reached the requested result and verdicts, then post-run rederivation found `runs/runs/MOT17-04-SDP/...`: controller and verifier had applied the same extra prefix. The single-root repair at `c4821e9e…` ran again and proved the corrected physical layout with the same green result/domain-only refusal. Its review then found that outer `run_evidence` recomputed policy-inventory digests but did not independently validate each `import_witness.json`; the child gated containment, but an independent envelope may not inherit the producer's answer. The verifier now schema-validates every completed run's witness, re-derives early-recorder/unbound/byte-containment rules against the archived RunSpec, and has an unbound-import refusal test. Both packets remain truthful superseded rehearsal evidence, not measurements. Each repair moves the 663-member execution projection, so a final disposable rehearsal is owed on the import-verifier head | yes |
