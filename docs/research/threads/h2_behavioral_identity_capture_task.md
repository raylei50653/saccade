...
| **W6a** | 🔧 **implementation — successor measurement authority envelope.** Review Correction 12 adds a successor-specific request/grant/receipt chain, v2 result/verification dispatch, an outer archive+run-evidence verifier, controlled-domain corpus admission and a production controller with no ledger override. The exclusive durable receipt is the sole consumption point and is written before producer entry. Naked measurement archives are refused. The accepted W5 witness exception-surface observation is repaired in the same projection-moving unit. Historical v1 archives remain on v1 dispatch; neither spent legacy grant is restored | no |
| **W6b** | 🔁 **two rehearsal reviews found two self-consistent verifier gaps; final rerun required.** The first disposable run at `84583eed…` reached the requested result and verdicts, then post-run rederivation found `runs/runs/MOT17-04-SDP/...`: controller and verifier had applied the same extra prefix. The single-root repair at `c4821e9e…` ran again and proved the corrected physical layout with the same green result/domain-only refusal. Its review then found that outer `run_evidence` recomputed policy-inventory digests but did not independently validate each `import_witness.json` or replay the run-derived pass predicates; the child gated both, but an independent envelope may not inherit the producer's answer. The verifier now schema-validates every completed run's witness, re-derives early-recorder/unbound/byte-containment rules against the archived RunSpec, replays A7.6 inventories/MOT/packets, and refuses a claimed pass that the archived runs do not reproduce. Tests pin an unbound import and changed MOT bytes. Both packets remain truthful superseded rehearsal evidence, not measurements. Each repair moves the 663-member execution projection, so a final disposable rehearsal is owed on the run-evidence-verifier head | yes |
