...
| **W6b** | 🔁 **final-projection rehearsal found one custody-layout defect; rerun required.** The first disposable run at `84583eed…` reached `measurement_pass`, `terminal: null`, valid inner and outer verdicts, and the expected domain-only corpus refusal. Post-run rederivation then found `runs/runs/MOT17-04-SDP/...`: the controller supplied an already-suffixed root to a retained driver that owns the `runs/` suffix, while the outer verifier repeated the same offset and therefore accepted the self-consistent error. The packet remains truthful superseded rehearsal evidence, not a formal measurement. The repair gives the driver the packet root and pins exactly one `runs/` layer in the verifier test; because both files are inside the execution closure, the projection moves and one new disposable rehearsal is owed on the repair head | yes |
