...
| **W5b** | **the observed import closure and namespace write-protection.** The child records the digests of every repository module it actually imported, and the verifier checks the declared closure against it — completeness measured against a source the record did not produce. The resolved namespace is write-protected for the duration of the run rather than sampled, since Correction 6 already disclaims pre/post equality as a transient-mutation detector | yes |
| **W5c** | successor archive admission in the corpus checker, and witness v3 | no |
| **W5d** | **Review Correction 11** describing what W5a–W5c landed, `sealed_prefix` re-pin, republish, governance closeout. Written after the code, per the order Correction 10 records | no |
