...
| **W6b** | ⏳ **required final-projection rehearsal.** On a clean committed implementation head, run the real controller and both independent closures once with an externally located disposable ledger and synthetic owner-shaped grant. Success requires `measurement_pass`, `terminal: null`, valid inner and outer verdicts, and one corpus refusal only for controlled-domain mismatch. This is not an owner grant, measurement or admission | yes |
