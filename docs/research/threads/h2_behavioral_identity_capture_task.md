...
| **W6a** | ✅ **implemented — successor measurement authority envelope.** Review Correction 12 adds a successor-specific request/grant/receipt chain, v2 result/verification dispatch, an outer archive+run-evidence verifier, controlled-domain corpus admission and a production controller with no ledger override. The exclusive durable receipt is the sole consumption point and is written before producer entry. Naked measurement archives are refused. The accepted W5 witness exception-surface observation is repaired in the same projection-moving unit. Historical v1 archives remain on v1 dispatch; neither spent legacy grant is restored. Final implementation projection at `f0ee5da3…`: 663-member closure `ca150e3b…`, projection `508f10f4…`, RunSpec `796d85f7…`; runtime identity changes only `identity_semantics` to `8fc9bd85…` (27 files), with probe `2dabed0b…` and equivalence `unproven` unchanged | no |
| **W6b** | ✅ **complete — final disposable-ledger rehearsal, after two review findings.** The first run at `84583eed…` exposed self-consistent `runs/runs/...` custody; the single-root run at `c4821e9e…` then exposed an outer verifier that inherited the child's import/A7.6 decisions. Both packets remain truthful superseded rehearsal evidence, not measurements. The final `f0ee5da3…` run independently validates the repaired boundaries: `measurement_pass`, `terminal: null`, 4/4 runs completed, five predicates pass, all MOT outputs byte-identical, every artifact digest recomputes, and each run carries 3,355 import observations with zero unbound and early recorder true. Inner v2 and outer envelope are each valid 7/7 with no verification-host inputs. The 517-file final packet closes under inventory digest `bc748a7a…`; canonical corpus refusal has exactly one reason, the disposable execution domain. The controlled ledger has no receipt for the synthetic grant. Custody: `/home/ray/h2_w6_successor_rehearsal_20260803T163028Z`. This satisfies the rehearsal gate only; it is not owner authority, measurement or admission | yes |
| **W6c** | ⛔ **formal execution stops at the owner gate.** Only after W6a review and W6b success may the exact clean merged head emit an authorization request. The owner must separately issue the matching third grant; no implementation, rehearsal, request or document may manufacture it. If consumed, the grant is spent even across a crash. Success disposition is owner-reviewed measurement closure plus canonical corpus admission; no Phase B, equivalence, `I`/`F`/`S`, seal, H0 re-entry or automatic successor follows | yes, but only after owner grant |
...
### W6 formal-measurement plan — owner-directed 2026-08-03

The owner separately selected a formal successor measurement plan after W5 was
closed. The plan reuses W5e's frozen content choice — MOT17-04-SDP, four ordered
runs and the 454-key RunSpec/capture ABI — but not W5e's diagnostic authority or
artifact. Review Correction 12 owns the new authority boundary. Implementation
may proceed through a disposable-ledger final-projection rehearsal; the stop
boundary is the request for a separately owner-issued third grant.

No grant exists in this repository or follows from this direction. Until the
owner hand-off occurs, the formal controller must not consume the controlled
ledger or run a measurement. The success target after a real grant is an
independently closed v2 envelope admitted by the canonical corpus and then
reviewed by the owner as H2 measurement closure. It deliberately ends Phase A:
Phase B, equivalence, sealing and later execution remain unauthorized.
