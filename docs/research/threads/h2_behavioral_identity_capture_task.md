...
| **W5c** | ✅ **landed — successor corpus admission and rehearsal witness v3.** The canonical corpus discovers successor archives from family-specific members rather than a directory-name prefix, independently recomputes the successor verifier verdict, and admits only a fully closed (`verification.json` + `checksums.sha256`), valid archive whose recorded authority is `exactly_once_measurement`. A producer-only directory is not evidence, and a green `non_qualifying_diagnostic` remains refused: archive validity and corpus admission are separate decisions. The authority spellings are now ruler-owned constants, not new tokens or a new grant. The rehearsal harness asks the real corpus owner and records its non-empty refusal in `h2_phase_a_rehearsal_witness_v3`; historical v1/v2 witnesses remain untouched. This is a structural provenance guard, **not** cryptographic authority proof; the repository has no signature mechanism. Contract evidence: successor measurement admission, diagnostic refusal, unclosed/invalid archive refusal, arbitrary-root discovery and corpus-owner rehearsal failure all pass. W5c moves the three-root execution closure: digest `4aa029b4…`, projection `93009c65…`, RunSpec `835a2110…` (659 members) | no |
| **W5d** | ✅ **landed — declaration and governance closeout.** Review Correction 11 describes the code after it landed: W5a's root closure, W5b's independent observed-import containment and immutable namespace, and W5c's validity-versus-admission boundary. The full pre-seal declaration is re-pinned at 114906 bytes / `3925b240…`; runtime identity is republished once from the existing read-only W4b probe and runtime-input records, with no controlled-host probe or native rebuild. `identity_semantics` moves `32bee186…` → `bfcb454d…`; decision surface, environment, implementation, runtime inputs and probe `2dabed0b…` remain unchanged, and equivalence remains unproven. Governance projections now point only to the final non-qualifying diagnostic W5e; no authorization, measurement, `I`/`F`/`S`, seal, corpus admission or equivalence follows | no |
| **W5e** | one diagnostic re-run, after every projection-moving edit has landed | yes |
