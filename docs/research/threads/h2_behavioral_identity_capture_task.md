...
| **W4b** | **bind the driver and run one diagnostic.** Needs a build, and therefore the rename-aside of `build/h2_layer_p` — never a delete. The owner's review points, to be answered by the implementation rather than by prose: the driver may only implement `Stages` / `Runs` and must select no terminal, reinterpret no finding and write no closure; the input monitor must still start before the binding in the real driver, or the stage-independent precedence W3 pinned holds only against the fakes; the diagnostic must stay non-qualifying even if every stage and run succeeds; the renamed-aside `build/h2_layer_p` must stay reconstructible — recorded path, bytes and time, never cleaned up, and never drawn into the new build's artifact roles or checksum scope; and all four runs must go through the same authority entry point, with `not_run` produced only by this module's contract control flow, never filled in by the driver | yes |
