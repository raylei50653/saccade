#pragma once

#include "saccade/common.hpp"
#include "perception/trt_engine.hpp"
#include <torch/script.h>
#include <torch/torch.h>
#include <memory>
#include <string>
#include <vector>

namespace saccade {

/**
 * @brief High-performance MambaGatedDetector utilizing TensorRT for YOLO backbone 
 * and LibTorch for Mamba SSM detection head and anchor box post-processing.
 */
class SACCADE_PERCEPTION_API MambaGatedDetector {
public:
    MambaGatedDetector(
        const std::string& trt_backbone_path,
        const std::string& mamba_head_script_path,
        int img_size = 640,
        float conf_thr = 0.05f
    );
    
    ~MambaGatedDetector();

    /**
     * @brief Execute end-to-end GPU inference pipeline
     * @param d_input_img  GPU Float Tensor of shape [1, 3, 640, 640] normalized to [0, 1]
     * @return GPU Float Tensor of shape [num_valid_dets, 6] (xyxy, score, class)
     */
    torch::Tensor forward(torch::Tensor d_input_img);
    torch::Tensor extract_fpn_embeddings(torch::Tensor d_boxes_xyxy);

    /**
     * @brief Raw pointer bindings for Python pybind11 integration
     */
    int forward_ptr(uintptr_t d_input_img, uintptr_t d_out_dets);
    void extract_fpn_embeddings_ptr(uintptr_t d_boxes_xyxy, int num_dets, uintptr_t d_out_embs);

    int get_img_size() const { return img_size_; }
    int get_fpn_dim() const { return 128; } // Traced JDE reduces dimension to 128

private:
    std::unique_ptr<TRTEngine> backbone_;
    torch::jit::script::Module mamba_head_;
    int img_size_;
    float conf_thr_;

    // Asynchronous custom CUDA stream for TensorRT
    cudaStream_t trt_stream_ = nullptr;

    // Device memory bindings for backbone features (P3, P4, P5)
    void* d_p3_ = nullptr;
    void* d_p4_ = nullptr;
    void* d_p5_ = nullptr;

    // Dimensions of FPN features
    const int p3_c_ = 128, p3_h_ = 80, p3_w_ = 80;
    const int p4_c_ = 256, p4_h_ = 40, p4_w_ = 40;
    const int p5_c_ = 512, p5_h_ = 20, p5_w_ = 20;
};

} // namespace saccade
