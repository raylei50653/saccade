...
class AdaptiveFramePool:
    def __init__(self, h, w, device='cuda'):
        ...
        self.tiles_batch6 = torch.zeros((6, 3, 640, 640), device=device, dtype=torch.float32)
        ...
        self.tile_3x2_dx = torch.tensor(
            [0., 160., 320., 0., 160., 320.], device=device, dtype=torch.float32
        ).view(6, 1, 1)
        self.tile_3x2_dy = torch.tensor(
            [0., 0., 0., 320., 320., 320.], device=device, dtype=torch.float32
        ).view(6, 1, 1)
