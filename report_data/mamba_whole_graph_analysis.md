...
|:--|-------|---------|:---:|------|
| **L1** | Whole-Detect Graph | resize → TRT backbone → Mamba head → postprocess decode | ~174 → 1 | detect: ~7.4ms → ~3.25ms |
...
Mamba head     (host)              │  postproc   │
   ↓ (174 次 PyTorch kernel launch) │  decode     │
input_proj     (kernel)            └─────────────┘
...
~174 次 kernel launch              1 次 graph replay
...
| **Mamba head export to TRT** | detect −30~50% | 將 PyTorch Mamba head 匯出為 TensorRT FP16 engine，消除 PyTorch dispatch overhead |
| Postprocess all-native | postprocess −0.17ms | 消除 python_tail 段，完全 C++/CUDA |
| 合併 postprocess 到 whole-detect graph | postprocess 1.84→0 | 將目前獨立的 postprocess 也納入 whole-detect graph |

### 8.3 Mamba Head TRT 現狀 (2026-06-06)

**已完成：**
- SelectiveScan TRT Plugin (`src/tracking/mamba_scan_plugin.cpp`) — 自定義 TensorRT IPluginV3，包裝 `selective_scan_fwd` CUDA kernel
- Plugin 共享庫 (`build/libsaccade_scan_plugin.so`, 170 KB) — 匯出 `getCreators`/`setLoggerFinder` 供 TRT 載入
...
- TRT Engine 建置 (`scripts/model/build_mamba_head_trt.py`) — ONNX → TRT FP16 engine (`models/yolo/mamba_head.engine`, 22.5 MB)
...
*最後更新：2026-06-06，基於 mamba_whole_graph 生產環境實測；含 TRT plugin 實作記錄*
