...
def main():
    ...
    print("=== Scanning results/mamba_sweep/ ===")
    mamba_sweep = results_dir / "mamba_sweep"
    if mamba_sweep.exists():
        for folder_path in sorted(mamba_sweep.iterdir()):
            if not folder_path.is_dir():
                continue
            hota = evaluate_folder(str(folder_path))
            if hota is not None:
                print(f"| mamba_sweep/{folder_path.name} | {hota['HOTA'] * 100:.1f}% |")

    print("\n=== Scanning results/mamba_sweep_low/ ===")
    mamba_sweep_low = results_dir / "mamba_sweep_low"
    if mamba_sweep_low.exists():
        for folder_path in sorted(mamba_sweep_low.iterdir()):
            if not folder_path.is_dir():
                continue
            hota = evaluate_folder(str(folder_path))
            if hota is not None:
                print(
                    f"| mamba_sweep_low/{folder_path.name} | {hota['HOTA'] * 100:.1f}% |"
                )
