...
## Where to look up a script

Every tracked script self-documents: a module docstring (`.py`) / description
comment (`.sh`) gives its **function**, and a `# status: <label>` header gives its
**triage state**. Two generated views are built from those headers by
`scripts/tools/build_scripts_index.py` (do not hand-edit the generated index blocks):

- **Global roll-up:** [`docs/ownership/scripts_inventory.generated.md`](../docs/ownership/scripts_inventory.generated.md)
  — all scripts grouped by directory, with status-label counts.
