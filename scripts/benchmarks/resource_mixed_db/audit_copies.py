"""Reject DB diagnostic GPU stamp transfers inside stable service."""
...
def check_copy(point, trace):
    size = point["arguments"]["bursts"] * point["arguments"]["units"] * 256 * 24
    copies = [row for row in trace["copies"] if row["bytes"] == size]
    final, spread = host_to_cupti(
        point["owner"]["calibration"], point["frames"][-1]["output"]
    )
