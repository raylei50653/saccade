"""Verify sealed DB runs and compare paired mixed-workload frontiers."""
...
from scripts.benchmarks.resource_mixed.report import digest  # noqa: E402
from scripts.benchmarks.resource_mixed_db.report import derive  # noqa: E402
...
def summarize(directory):
    ...
    for name, value in manifest["source_sha256"].items():
        if digest(directory / "sources" / name) != value:
    ...
    for entry in manifest["schedule"]:
        ...
        saved = json.loads((directory / entry["name"] / "report.json").read_text())
        if row != saved or not row["valid"]:
            raise ValueError("saved metric mismatch or replay failure")
        ...
        signature = dict(
            ...
            priorities=[stream["priority"] for stream in owner["streams"]],
            ...
            db_enabled=point["route"]["enabled"],
            db_owned_class=point["route"]["owned_class"],
        )
        if entry["audit"]:
        ...
        rows.append(row)
    values = manifest["arguments"]
    expected = {
        (rep, policy, iterations, window, audit)
        for rep in range(values["repeats"])
    ...
    for dynamic in measurements:
        ...
        controls = {
            ...
            if row["rep"] == dynamic["rep"]
            and row["iterations"] == dynamic["iterations"]
            and row["window"] == dynamic["window"]
        }
        for policy in ("fixed", "shared"):
            ...
            comparisons.append(
                dict(
                    ...
                    db_fps_ratio=dynamic["db_fps"] / other["db_fps"],
                    period_p99_ratio=dynamic["period_ms"]["p99"]
                    / other["period_ms"]["p99"],
    ...
    return dict(
        schema="saccade-mixed-db-summary-v1",
...
def main():
    ...
    print(
        f"Verified {len(result['rows'])} runs; "
        f"{len(result['comparisons'])} paired comparisons"
    )
