"""Freeze, execute and seal repeated double-buffer mixed-workload comparisons."""
...
def main():
    ...
    manifest = dict(
        ...
        library_sha256={path.name: digest(path) for path in output.glob("*.so")},
    ...
    for row in schedule:
        command = [
            sys.executable,
            "-u",
            str(ROOT / "scripts/benchmarks/resource_mixed_db/run_point.py"),
            "--policy",
            row["policy"],
            "--window",
            str(row["window"]),
