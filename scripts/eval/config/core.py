...
def add_core_args(parser: argparse.ArgumentParser) -> None:
    ...
    mod.add_argument("--module-detection", default=None, metavar="PATH",
                     help="YAML for detection/preprocessing params (tiling, gamma, FP filter, …).")
    mod.add_argument("--module-geometry", default=None, metavar="PATH",
                     help="YAML for geometry priors, ID stability, Kalman, quality scaling.")
    mod.add_argument("--module-reid", default=None, metavar="PATH",
                     help="YAML for ReID backbone, budget, crop, lazy-ReID, async pipeline.")
    mod.add_argument("--module-semantic", default=None, metavar="PATH",
                     help="YAML for semantic relink, appearance bank, clean filter.")
    mod.add_argument("--module-trigger", default=None, metavar="PATH",
                     help="YAML for dynamic ReID trigger policy (Experimental).")
    mod.add_argument("--module-lifecycle", default=None, metavar="PATH",
    ...
    core.description = "Primary thresholds and association controls — tune every ablation."
    core.add_argument("--conf-threshold", type=float, default=0.05,
                      help=_help("Detector confidence floor.", range_hint="0-1, usually 0.01-0.3",
                                 edge="lower increases recall/noise; higher suppresses weak people"))
    core.add_argument("--track-thresh", type=float, default=0.05,
                      help=_help("Low-confidence association floor.", range_hint="0-1, usually <= high-thresh"))
    core.add_argument("--high-thresh", type=float, default=0.45,
                      help=_help("High-confidence matching threshold.", range_hint="0-1",
                                 edge="higher reduces false links but raises fragmentation"))
    core.add_argument("--mid-thresh", type=float, default=0.10,
                      help=_help("Mid-tier confidence bucket.", range_hint="0-1",
                                 edge="keep between track-thresh and high-thresh"))
    core.add_argument("--new-track-thresh", type=float, default=0.35,
                      help=_help("Minimum score for starting a new track.", range_hint="0-1",
                                 edge="higher suppresses weak true positives"))
    core.add_argument("--match-thresh", type=float, default=0.75,
                      help=_help("Association similarity gate.", range_hint="0-1",
                                 edge="near 1 favors purity over continuity"))
    core.add_argument("--confirm-streak", type=int, default=1,
                      help=_help("Hits needed before confirming a tentative track.", range_hint=">=1",
                                 edge="higher removes flicker but delays births"))
    core.add_argument("--confirm-score-thresh", type=float, default=0.0,
                      help=_help("Minimum score for counting toward confirmation.", range_hint="0-1"))
    core.add_argument("--adaptive-confirmation", action=argparse.BooleanOptionalAction, default=False,
                      help="Enable score/geometry-aware confirmation logic.")
    core.add_argument("--gmc", action=argparse.BooleanOptionalAction, default=True,
                      help="Enable global motion compensation.")
    core.add_argument("--gmc-mode", choices=["cpu", "gpu"], default="gpu",
                      help="GMC algorithm mode. 'cpu' uses OpenCV LK; 'gpu' uses cuFFT phase correlation.")
    core.add_argument("--gmc-downscale", type=int, default=8,
                      help=_help("Downscale factor for GMC estimation.", range_hint=">=1",
                                 edge="too large loses fine camera motion"))
    core.add_argument("--per-seq-adapt", action=argparse.BooleanOptionalAction, default=True,
                      help="Auto-scale time-like tracker parameters by sequence frameRate.")
    core.add_argument("--warmup-frames", type=int, default=50,
                      help=_help("Frames to ignore for warmup effects.", range_hint=">=0"))
    core.add_argument("--profile-stages", action="store_true",
                      help="Profile stage-level runtime.")
