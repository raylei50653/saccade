...
class CoordinateOptimizer:
    def __init__(self, args):
        ...
        if not self.log_file.exists() or args.fresh:
            with open(self.log_file, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["timestamp", "round", "parameter", "action", "value", "idf1", "mota", "ids", "score", "is_best"])

        # Shared models (In-process optimization)
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Pre-loading models to memory...")
    ...
    def _load_history(self):
        try:
            with open(self.history_file, "r") as f:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Loaded history. Best value: {self.best_value:.4f}")
    ...
    def _log_trial(self, round_idx, param_name, action, param_val, metrics, score, is_best):
    ...
    def optimize(self):
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚀 Starting Coordinate Optimization (In-Process)...")
        print(f"  Target: {len(self.all_params)} parameters, {self.args.rounds} rounds.")
        
        if self.best_value == -float('inf'):
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Running initial evaluation...")
            metrics = self.run_eval_internal(self.current_params)
            if metrics:
                self.best_value = self.calculate_value(metrics)
                self.best_metrics = metrics
                print(f"  Initial Value: {self.best_value:.4f} (IDF1={metrics['IDF1']}, MOTA={metrics['MOTA']}, IDs={metrics['IDs']})")
                self._log_trial(0, "INITIAL", "STAY", "N/A", metrics, self.best_value, True)
        ...
        for r in range(self.args.rounds):
            ...
            for name in param_names:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Optimizing {name} (current: {curr_val})")
                ...
                for cand_val, label in candidates:
                    if "threshold" in name or "thresh" in name:
                        if cand_val < 0 or cand_val > 1: continue
                    ...
                    self._log_trial(r+1, name, label, cand_val, metrics, val, is_new_best)
                    
                    if is_new_best:
                        print(f"    ✅ Improvement ({label}): {self.best_value:.4f} -> {val:.4f} (IDF1: {metrics['IDF1']})")
                        self.best_value = val
...
if __name__ == "__main__":
    ...
    if args.params:
        ...
        optimizer.all_params = {k: v for k, v in optimizer.all_params.items() if k in target_params}
