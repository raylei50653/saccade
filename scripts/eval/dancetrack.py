...
def _load_config_defaults(project_root: Path) -> dict:
    ...
    if pre_args.config:
        ...
        fallback = project_root / "configs" / "dancetrack.yaml"
        if fallback.exists():
...
if __name__ == "__main__":
    parser = build_parser()
    # Default data root for DanceTrack
    parser.set_defaults(data_root="datasets/DanceTrack/val")
    parser.set_defaults(
        split=""
    )  # val set is directly under the dir usually or in 'val'
        ...
        if data_root.exists():
            args.sequences = ",".join(
                sorted(
                    ...
                    if d.is_dir() and d.name.startswith("dancetrack")
                )
            )
