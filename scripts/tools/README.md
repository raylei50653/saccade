...
| `build_runtime_identity.py` | stable | cli | Build the published four-axis `runtime_identity` of the online track. |
...
| `check_runtime_identity_staleness.py` | stable | cli | Flag version lag between the online track and the research claims that cite it. |
...
| `h2_behavioral_identity.py` | stable | cli | Compute the H2 `behavior` axis: a policy-visible digest of one eval run. |
| `h2_path_partition.py` | stable | cli | H2 firewall: classify repository paths and decide Layer-P retry admissibility. |
