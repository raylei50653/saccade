...
def cuda_letterbox(src: torch.Tensor, dst_size: int, pad_val: float = 114.0 / 255.0):
    ...
    x_off = (dst_size - w_new) // 2
    canvas = torch.empty(
        (3, dst_size, dst_size), device=src.device, dtype=torch.float32
    )
...
def test_case(label, h_orig, w_orig, dst_size, max_err=1e-6):
    ...
    print(
        f"  [{label}] h={h_orig} w={w_orig} → {dst_size}×{dst_size}  "
        f"max={max_abs:.2e}  mean={mean_abs:.2e}  pad_err={pad_err:.2e}",
        end="",
    )
