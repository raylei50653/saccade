...
TEACHER="${TEACHER:-runs/gated_det_pp22/best.ckpt}"   # e30 best-by-loss
CACHE=runs/mamba_teacher_cache_pp22_e30
SEQS="$(paste -sd, datasets/PersonPath22/train_seqs.txt)"
LOG=runs/pp22_heldout_e30.log
...
$PY scripts/train/temporal_yolo/train_mamba_head.py "${common[@]}" \
    --cache-dir "$CACHE" --run-dir runs/mamba_distill_pp22_e30 \
...
$PY scripts/train/temporal_yolo/train_mamba_gt.py "${common[@]}" \
    --mamba-ckpt runs/mamba_distill_pp22_e30/best.ckpt \
    --run-dir runs/mamba_gt_pp22_stage1_e30 \
...
$PY scripts/train/temporal_yolo/train_mamba_gt.py "${common[@]}" \
    --cache-dir "$CACHE" --mamba-ckpt runs/mamba_gt_pp22_stage1_e30/best.ckpt \
    --run-dir runs/mamba_gt_pp22_t3_e30 \
...
$PY scripts/train/temporal_yolo/train_mamba_gt.py "${common[@]}" \
    --cache-dir "$CACHE" --mamba-ckpt runs/mamba_gt_pp22_t3_e30/best.ckpt \
    --run-dir runs/mamba_gt_pp22_t3_t1_e30 \
    --clip-len 1 --clip-stride 2 --lr 1e-4 --lr-gate 0 --gt-ratio 0 \
    --scan-stop-grad --d-state 16 --d-model 128 --epochs 15 --warmup-epochs 3 \
    --best-by train-loss --seed 42 --no-preload-cache 2>&1 | tee -a "$LOG"

echo "[done] final ckpt=runs/mamba_gt_pp22_t3_t1_e30/best.ckpt  $(date)" | tee -a "$LOG"
