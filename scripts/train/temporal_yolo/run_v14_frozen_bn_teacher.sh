...
# the gated teacher with --lr-yolo 0 --adapt-bn (weights + BN affine frozen, BN running stats adapt
# at stock yolo26s; only the spatial gate learns). Everything downstream — cache,
# legacy N=1 distill, N=1 GT warm-start, fixed N=16 GT adaptation, eval — is
# byte-for-byte identical to the e30 runner.
#
# Purpose: decompose "teacher prior" (success factor #3) into its two unmeasured
# halves — the 1.84% YOLO weight drift vs the trained gate. If recall/IDF1 hold
# with a frozen stock backbone, the deployed YOLO layer never has to move and the
# stock TRT engine can be reused without re-export.
#
# Compare against runs/gated_det_v14_full_e30 lineage (the lr-yolo 1e-5 arm).
set -euo pipefail
...
SEED="${SEED:-42}"
RUN_PREFIX="${RUN_PREFIX:-v14_frozenbn}"
PRINT_ONLY=0
...
# STAGE T — sole controlled variable vs the frozen-YOLO arm: add --adapt-bn so BN
# running stats adapt to MOT while weights + BN affine stay frozen at stock
# yolo26s; only the spatial gate trains. Isolates BN domain-shift recalibration
# from weight learning. Frozen weights => no epoch-20 NaN; runs cleanly to 30.
run_training_stage "STAGE T: gated teacher e30 (FROZEN W + BN-ADAPT)" "$TEACHER_RUN" 30 \
...
echo "=== V14 FROZEN-W + BN-ADAPT TEACHER CONTROL COMPLETE ==="
echo "teacher: $TEACHER (lr-yolo 0 --adapt-bn: stock weights, MOT-adapted BN stats)"
echo "final:   $GT_N16/best.ckpt"
echo "recall:  $REPORT"
echo "track:   $RESULTS"
echo "decompose: frozen (runs/mamba_gt_v14_frozenyolo_n16, IDF1 55.8) <= this <= lr-yolo 1e-5"
