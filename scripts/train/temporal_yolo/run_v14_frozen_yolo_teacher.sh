...
# the gated teacher with --lr-yolo 0 (backbone + detect + BatchNorm stats frozen
...
SEED="${SEED:-42}"
RUN_PREFIX="${RUN_PREFIX:-v14_frozenyolo}"
PRINT_ONLY=0
...
# STAGE T — sole controlled variable: --lr-yolo 0 freezes backbone+detect+BN at
# stock yolo26s; only the spatial gate trains. With a frozen backbone the teacher
# does not hit the epoch-20 NaN of the lr-yolo 1e-5 arm, so it runs cleanly to 30.
run_training_stage "STAGE T: gated teacher e30 (FROZEN YOLO)" "$TEACHER_RUN" 30 \
  ...
  --protocol-revision V14-FROZENYOLO-TEACHER-20260614
...
echo "=== V14 FROZEN-YOLO TEACHER CONTROL COMPLETE ==="
echo "teacher: $TEACHER (lr-yolo 0, stock backbone)"
echo "final:   $GT_N16/best.ckpt"
echo "recall:  $REPORT"
echo "track:   $RESULTS"
echo "compare against the lr-yolo 1e-5 arm: runs/mamba_gt_v14_full_e30_n16"
