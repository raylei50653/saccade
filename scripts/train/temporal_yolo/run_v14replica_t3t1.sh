...
# Phase A: 15ep GT2 with temporal (T=3), Phase B: 15ep GT2 T=1 re-adaptation.
...
echo ""
echo "=== PHASE B: T=1 spatial re-adaptation (15 epochs) ==="
.venv/bin/python scripts/train/temporal_yolo/train_mamba_gt.py \
    --data-root datasets/MOT17 \
    --teacher-ckpt "$TEACHER" \
    --mamba-ckpt runs/mamba_gt_v14replica_t3/best.ckpt \
    --cache-dir "$CACHE" \
    --run-dir runs/mamba_gt_v14replica_t3_t1 \
...
.venv/bin/python scripts/eval/mamba_size_binned_recall.py \
    ...
    --output report_data/mamba_size_recall_v14replica_t3t1_02.json

echo ""
echo "=== TRACKING EVAL ==="
.venv/bin/python scripts/eval/mot17.py --preset mamba_whole_graph --detector SDP \
    --mamba-ckpt runs/mamba_gt_v14replica_t3_t1/best.ckpt \
    --output results/mamba_v14replica_t3t1
