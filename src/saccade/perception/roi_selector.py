...
class ROISelector:
    ...
    def __init__(
        self, grid_size: int = 4, frame_width: int = 1920, frame_height: int = 1080
    ) -> None:
        self.grid_size = grid_size
        self.w = frame_width
        self.h = frame_height
        self.last_roi_center: Tuple[float, float] = (
            float(frame_width) / 2.0,
            float(frame_height) / 2.0,
        )

    def select_best_roi(
        self, tracks: torch.Tensor, roi_size: int = 640, smoothing: float = 0.5
...
if __name__ == "__main__":
    ...
    dummy_tracks = torch.tensor(
        [[100, 100, 200, 200], [150, 150, 250, 250], [50, 50, 150, 150]],
        device="cuda",
        dtype=torch.float32,
    )
