...
def benchmark_drift_handler(num_iterations=1000, batch_size=8):
    ...
    # 模擬 BBox [x1, y1, x2, y2]
    boxes = torch.tensor([[0, 0, 100, 100]] * batch_size, device="cuda", dtype=torch.float32)
    ...
    print("═" * 60)
    print(f"Filter Throughput: { (1000 / avg_latency_filter) * batch_size:,.0f} objects/sec")
    print(f"Update Throughput: { (1000 / avg_latency_update) * batch_size:,.0f} objects/sec")
