...
echo -e "\n[1/5] Testing Core Transport Latency..."
uv run python tests/benchmarks/bench_core_transport.py

# 2. 感知全鏈路 (E2E Pipeline Stress - YOLO26 + SigLIP 2)
echo -e "\n[2/5] Testing Full Perception Pipeline (L1-L2)..."
uv run python tests/benchmarks/bench_perception_pipeline.py

# 3. 模型對比 (YOLO11 vs YOLO26)
echo -e "\n[3/5] Testing Model Generation Comparison..."
uv run python tests/benchmarks/bench_model_comparison.py

# 4. 語義漂移 (Drift Handler Efficiency)
echo -e "\n[4/5] Testing Semantic Drift Filtering..."
uv run python tests/benchmarks/bench_drift_handler.py

# 5. 存儲效能 (ChromaDB Vector Store)
echo -e "\n[5/5] Testing Vector Storage Performance..."
