...
from saccade.perception.eval.relink import IdentityResolver as PyIdentityResolver, PythonSemanticRelinker
from saccade.perception.eval.runner import TrackletLifecycleMerger as PyLifecycleMerger
...
def _make_py_lifecycle(**kw):
    defaults = dict(enabled=True, ttl=20, min_gap=1, spatial_gate=0.30,
                    min_iou=0.10, sim_threshold=0.50, require_embedding=False, ema=0.70)
    defaults.update(kw)
...
def _make_py_relinker(**kw):
    defaults = dict(sim_threshold=0.70, ttl=30, ema_beta=0.80, spatial_gate=0.30,
                    min_lost_frames=1, min_iou=0.10, mahalanobis_threshold=0.0)
    defaults.update(kw)
...
def _frames(rng: torch.Generator, n_frames: int, n_per: int, ids: list[int],
            frame_w=640, frame_h=480, emb_dim=8):
    frames = []
    for _ in range(n_frames):
        ...
        for k in range(n_per):
            ...
            batch.append((ids[k % len(ids)], (x1, y1, x2, y2), float(torch.rand(1, generator=rng).item() * 0.5 + 0.5), emb))
        frames.append(batch)
    return frames
...
def _run_lifecycle_many(lc, frames, frame_w, frame_h):
    ...
    for frame_id, batch in enumerate(frames, start=1):
        ...
        ids = lc.resolve_many_packed(local_ids, boxes, scores, embeddings,
...
def test_lifecycle_stats_match():
    rng = torch.Generator(); rng.manual_seed(13)
...
def test_lifecycle_none_embeddings():
    ...
    scores = [0.9, 0.8]
    embs = [None, None]
    ids_py  = [int(i) for i in py_lc.resolve_many_packed([1, 2], boxes, scores, embs,
                                                           frame_id=1, frame_w=640, frame_h=480)]
    ids_cpp = [int(i) for i in cpp_lc.resolve_many_packed([1, 2], boxes, scores, embs,
                                                            frame_id=1, frame_w=640, frame_h=480)]
...
def _run_resolver(resolver, lc, frames, frame_w, frame_h):
    ...
    for frame_id, batch in enumerate(frames, start=1):
        ...
        boxes      = [b[1] for b in batch]
        scores     = [b[2] for b in batch]
        ids = resolver.resolve_pass(local_ids, embs, boxes, scores,
                                    frame_id=frame_id, frame_w=frame_w, frame_h=frame_h)
        all_ids.append([int(i) for i in ids])
        lc.prune(frame_id)
    return all_ids
...
def test_identity_resolver_cpp_vs_python_ids():
    ...
    py_rk  = _make_py_relinker();  py_lc  = _make_py_lifecycle()
    cpp_rk = _make_cpp_relinker(); cpp_lc = _make_cpp_lifecycle()
...
def test_identity_resolver_cpp_vs_python_alias_and_stats():
    ...
    assert dict(py_rk.alias)  == dict(cpp_rk.alias),  f"relinker alias mismatch"
    assert dict(py_lc.alias)  == dict(cpp_lc.alias),  f"lifecycle alias mismatch"
    assert dict(py_rk.stats)  == dict(cpp_rk.stats),  f"relinker stats mismatch"
    assert dict(py_lc.stats)  == dict(cpp_lc.stats),  f"lifecycle stats mismatch"
...
def test_identity_resolver_cpp_relink_match_scenario():
    ...
    def run(resolver, lc):
        ...
        resolver.resolve_pass([1], [emb_a], [box_a], [0.9],
                               frame_id=1, frame_w=640, frame_h=480)
        lc.prune(1)
        # Frame 6: re-appear with similar embedding — relink candidate
        result = resolver.resolve_pass([99], [emb_b], [box_a], [0.85],
                                        frame_id=6, frame_w=640, frame_h=480)
        lc.prune(6)
